...
namespace Safra.Api.Controllers.V1
{
    ...
    {
        private IClienteAppServices _service;
        public ClienteController(IClienteAppServices service)
        {
            _service = service;
        }
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult> Insert([FromBody] ClienteViewModelPost model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                ...
                if (result != null)
                {
                    return Created(new Uri(Url.Link("FindtWithId", new { id = result.idCliente })), result);
                }
                else
                {
                    return BadRequest();
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult> Update([FromBody] ClienteViewModelPut model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
